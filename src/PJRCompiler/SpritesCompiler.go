...
func (compiler *PJRCompiler) compileSprites(labelSection []string) ([]byte, error) {
		...
		if len(v) > 0 {
			if isCorrectLabelInformation(v) {
				...
				if len(labelName) > 0 && len(labelPath) > 0 {
					id := uint32(len(compiler.sectionInfo.SpriteTags))
					err := compiler.sectionInfo.AddSpriteTag(labelName, id)
					if err != nil {
						return nil, err
					}
